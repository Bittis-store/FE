...
import { Popconfirm, Space, TableProps, Tooltip } from 'antd';
...
const ActionLink = ({ onClick, children }: { onClick?: () => void; children: React.ReactNode }) => (
    <p className='cursor-pointer text-blue-500 transition-colors duration-500 hover:text-blue-400' onClick={onClick}>
        {children}
    </p>
);
    ...
    return useMemo(
        () => [
            ...
            },
            {
                title: 'Giá tiền (VNĐ)',
                key: 'price',
                render: (_, record) => <>{Currency.format(record.price)}</>,
            },
            {
                title: 'Kho hàng',
                key: 'stock',
                render: (_, record) => {
                    const totalStock = record?.variants.reduce((acc, curr) => acc + (curr?.stock || 0), 0);
                    return (
                        <>
                            <div className='flex flex-col justify-between'>
                                <p className='h-14 whitespace-nowrap'>
                                    Tổng: {totalStock > 0 ? totalStock : <span className='text-red-500'>Hết hàng</span>}
                                </p>
                            </div>
                            <div className=''>
                                {record.variants.map((item, index) => (
            ...
            },
            {
                ...
                render: (_, record) => <h4>{record.tags.map((item: any) => item.name).join(', ')}</h4>,
            },
            {
                title: 'Danh mục',
                key: 'category',
                ...
                render: (_, record) => (
                    <h4>{typeof record.category === 'object' ? record.category.name : record.category}</h4>
                ),
            },
            {
                title: 'Trạng thái',
                        ...
                        {record.isActive ? (
                            ...
                            <Tooltip title='Hiện thị sản phẩm này'>
                                <Popconfirm
                                    ...
                                    cancelText='Đóng'
                                >
                                    <ActionLink>Hiển thị</ActionLink>
                                </Popconfirm>
                            </Tooltip>
                        )}
                    </Space>
        ...
        [categoriesFilter, tagsFilter, getColumnSearchProps, getFilteredValue, mutateHideProduct, mutateShowProduct]
    );
};
